...
from ldm.models.diffusion.attention_manager import manager
...
class DPMSolverSampler(manager):
    ...
    def sample(self,
        ...
        self.dpm_solver = DPM_Solver(model_fn, self.noise_schedule, algorithm_type="dpmsolver++", correcting_xt_fn=correcting_xt_fn, model=self.model, **kwargs)
    
        x, intermediates = self.dpm_solver.sample(
            img, t_start=t_start, t_end=t_end, steps=S, skip_type=skip_type,
            method=method, order=order,
            lower_order_final=lower_order_final, return_intermediate=True, **kwargs
            )
